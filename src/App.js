...
const SelectionControls = ({ files, selectedFiles, onSelectAll, onClearSelection }) => (
    <div className="flex items-center gap-2 mb-4 p-2 bg-blue-50 rounded-lg">
      <button onClick={onSelectAll} className="text-sm text-blue-600 hover:text-blue-800">
...
const FolderTree = ({ folderTree, currentFolder, setCurrentFolder, setContextMenu, onCreateFolder }) => (
  <div className="w-64 bg-gray-50 border-r p-4 overflow-y-auto">
    <div className="flex items-center justify-between mb-4">
      <h3 className="font-semibold text-gray-800">Folders</h3>
      <button onClick={onCreateFolder} className="text-blue-600 hover:text-blue-800 text-sm font-medium" title="Create New Folder">
        + New
      </button>
    </div>
        ...
        {files.map(file => (
          <div key={file.id} className={`relative bg-white border-2 rounded-lg p-3 hover:shadow-lg cursor-pointer transition-all duration-200 group ${isSelected(file) ? 'border-blue-500 bg-blue-50 shadow-md' : 'border-gray-200 hover:border-gray-300'}`} onContextMenu={e => onFileRightClick(e, file)} onClick={() => onFileClick(file)}>
            <div className="absolute top-2 left-2 z-10">
              <input type="checkbox" checked={isSelected(file)} onChange={e => onFileSelect(file, e)} className="rounded shadow-sm" />
            </div>
...
const FileDetailsModal = ({ file, isOpen, onClose, onUpdate, onDelete }) => {
  ...
  useEffect(() => {
    if (file) {
      setEditData({ title: file.title || '', description: file.description || '', notes: file.notes || '', tags: file.tags || '', station: file.station || '', category: file.category || '' });
    }
  }, [file]);

  const handleSave = () => {
    onUpdate(file.id, { 'Title': editData.title, 'Description': editData.description, 'Notes': editData.notes, 'Tags': editData.tags, 'Station': editData.station, 'Category': editData.category });
    setIsEditing(false);
  };
      ...
      <div className="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl">
        ...
        <div className="flex h-[calc(90vh-120px)]">
          <div className="flex-1 p-6 bg-gray-50 flex items-center justify-center">
            ...
            {file.type === 'video' && file.url && (<video src={file.url} controls className="max-w-full max-h-full rounded-lg shadow-sm">Your browser does not support video playback.</video>)}
  ...
  return (
    <div className="fixed bottom-4 right-4 bg-white border border-gray-300 rounded-lg shadow-xl p-4 w-80 z-40">
      ...
      <div className="space-y-3">
        <div className="flex gap-2">
          <select value={batchAction} onChange={e => setBatchAction(e.target.value)} className="flex-1 p-2 border border-gray-300 rounded text-sm">
        ...
        {batchAction === 'update' && (
          <div className="space-y-2">
            <select value={batchData.category} onChange={e => setBatchData({...batchData, category: e.target.value})} className="w-full p-2 border border-gray-300 rounded text-sm"><option value="">Category (no change)</option><option value="Images">Images</option><option value="Video">Video</option><option value="Audio">Audio</option><option value="Documents">Documents</option><option value="Files">Files</option></select>
            <input type="text" placeholder="Tags (append/replace)" value={batchData.tags} onChange={e => setBatchData({...batchData, tags: e.target.value})} className="w-full p-2 border border-gray-300 rounded text-sm" />
            <input type="text" placeholder="Station" value={batchData.station} onChange={e => setBatchData({...batchData, station: e.target.value})} className="w-full p-2 border border-gray-300 rounded text-sm" />
            <button onClick={handleBatchUpdate} className="w-full px-3 py-2 bg-blue-600 text-white rounded text-sm hover:bg-blue-700">Update {selectedFiles.length} Files</button>
          </div>
        )}
        {batchAction === 'move' && (
          <div className="space-y-2">
            ...
            <button onClick={() => onBatchMove(selectedFiles, batchData.category)} disabled={!batchData.category} className="w-full px-3 py-2 bg-green-600 text-white rounded text-sm hover:bg-green-700 disabled:bg-gray-400">Move {selectedFiles.length} Files</button>
          </div>
        )}
        {batchAction === 'delete' && (
          <div className="space-y-2">
            <p className="text-sm text-red-600">This will permanently delete {selectedFiles.length} files.</p>
            <button onClick={() => onBatchDelete(selectedFiles)} className="w-full px-3 py-2 bg-red-600 text-white rounded text-sm hover:bg-red-700">🗑️ Delete {selectedFiles.length} Files</button>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="fixed inset-0 z-40" onClick={onClose}>
      <div className="fixed bg-white border border-gray-300 rounded-lg shadow-xl py-2 z-50 min-w-48" style={{ left: contextMenu.x, top: contextMenu.y }}>
        {contextMenu.type === 'file' ? (
          <>
            <button className="w-full px-4 py-2 text-left hover:bg-gray-100 text-sm flex items-center gap-2" onClick={() => handleAction('view')}>👁️ View Details</button>
            <button className="w-full px-4 py-2 text-left hover:bg-gray-100 text-sm flex items-center gap-2" onClick={() => handleAction('download')}>💾 Download</button>
            <button className="w-full px-4 py-2 text-left hover:bg-gray-100 text-sm flex items-center gap-2" onClick={() => handleAction('rename')}>✏️ Rename</button>
            <button className="w-full px-4 py-2 text-left hover:bg-gray-100 text-sm flex items-center gap-2" onClick={() => handleAction('move')}>📁 Move to Category</button>
            <hr className="my-1" />
            <button className="w-full px-4 py-2 text-left hover:bg-red-50 text-sm text-red-600 flex items-center gap-2" onClick={() => handleAction('delete')}>🗑️ Delete</button>
          </>
        ) : (
          <>
            <button className="w-full px-4 py-2 text-left hover:bg-gray-100 text-sm flex items-center gap-2" onClick={() => handleAction('rename')}>✏️ Rename Folder</button>
            <button className="w-full px-4 py-2 text-left hover:bg-red-50 text-sm text-red-600 flex items-center gap-2" onClick={() => handleAction('delete')}>🗑️ Delete Folder</button>
          </>
        )}
      </div>
    </div>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Station</label><input type="text" value={formData.station} onChange={e => handleChange('station', e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="e.g., Studio A, Location B" /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Description</label><textarea value={formData.description} onChange={e => handleChange('description', e.target.value)} rows={3} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Brief description of the files..." /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Notes</label><textarea value={formData.notes} onChange={e => handleChange('notes', e.target.value)} rows={2} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Additional notes..." /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Tags</label><input type="text" value={formData.tags} onChange={e => handleChange('tags', e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="tag1, tag2, tag3" /></div>
